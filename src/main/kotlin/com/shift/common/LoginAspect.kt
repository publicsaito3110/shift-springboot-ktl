...
class LoginAspect {
     ...
     *
     *
     * Controller実行前にセッション及びURI制限を行う<br></br>
     * セッションが未保持のとき、<br></br>
     * ・セッションの未保持が許容されていないURI: ログイン画面へ強制的に遷移<br></br>
     * ・セッションの未保持が許容されているURI: そのまま実行<br></br>
     * セッションが保持されているとき、<br></br>
     * ・ログイン関連のURI: 強制的にホーム画面へ遷移<br></br>
     * ・ログイン関連以外のURI: そのまま実行
     *
     *
    ...
    fun executeSession(joinPoint: ProceedingJoinPoint): Any? {
        ...
        val isA: Boolean = requestMappingHandler.useTrailingSlashMatch()
